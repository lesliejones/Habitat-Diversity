...
fviz_pca_ind(res.pca, geom.ind = "", axes = c(1,2), 
             repel = TRUE, habillage = Phab22$region, addEllipses = TRUE, label="none", title="")

print(res.pca)

#Use habitat indicator coordinates to select indicators for model - choose all indicators > 0.3
pcvar <- get_pca_var(res.pca)
write.csv(pcvar$coord, file="C:/Habitat_Diversity/Analysis/Data/sock_indicator_scores.csv")
...
myvar<- c("slope_avgW", "slope_cvW", "elev_avgW", "elev_cvW", "forest", "shrub", "wetland", "ccomplexity", "sinuosity", "floodplainA", "flood_elev", "lake_elev", "glacierA","elev_avgS", "elev_cvS", "slope_avgS", "slope_cvS", "sockeye_hab", "region", "lakeD")

socksub <- pcout[myvar]
#chart.Correlation(socksub, histogram=TRUE, pch=19)

fitsock <- lm(sockeye_hab ~ slope_avgW + slope_cvW + elev_avgW + elev_cvW + forest + shrub + wetland + ccomplexity + sinuosity + floodplainA + flood_elev + lake_elev + lakeD + glacierA + elev_avgS + elev_cvS + slope_avgS + 
                slope_cvS  , data = socksub)
...
library(car)
vif(lm(sockeye_hab ~ slope_avgW + slope_cvW + elev_cvW + forest + shrub + wetland + floodplainA + flood_elev +  lakeD + glacierA + elev_avgS + elev_cvS + slope_avgS, data=socksub))
#remove slope_cvW bc VIF >5
vif(lm(sockeye_hab ~ slope_avgW + elev_cvW + forest + shrub + wetland + floodplainA + flood_elev +  lakeD + glacierA + elev_avgS + elev_cvS + slope_avgS, data=socksub))

library(lme4)
fitsock2 <- lmer(sockeye_hab ~ slope_avgW + elev_cvW + forest + shrub + wetland + floodplainA + flood_elev +  lakeD + glacierA + elev_avgS + elev_cvS + slope_avgS+ (1|region) , data = socksub)
summary(fitsock2)
...
r2sock <- rsquared(lme4::lmer(sockeye_hab ~ slope_avgW + elev_cvW + forest + shrub + wetland + floodplainA + flood_elev +  lakeD + glacierA + elev_avgS + elev_cvS + slope_avgS+ (1|region) , data=socksub))
...
sock_pred <- predict(fitsock2, Phab3, allow.new.levels=TRUE)
View(sock_pred)
sock_out <- cbind(Phab3,sock_pred) 
View(sock_out)
sock_out$sock_pred2=(sock_out$sock_pred*100)
sock_out$sock_hab2 = (sock_out$sockeye_hab*100)
sock_out$sock_diff = (sock_out$sockeye_hab - sock_out$sock_pred)
View(sock_out)
sock_out2 <- sock_out

ggplot(sock_out2, aes(x=sock_pred, y=sockeye_hab)) + geom_point()

#export this and join with watersheds in arcgis
write.csv(sock_out2, file="C:/Habitat_Diversity/Analysis/Data/pred_sock.csv")
```
#pca plots
```{r,  warning=FALSE,  message=FALSE}
