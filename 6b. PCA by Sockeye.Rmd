...
#View(Phab)
Phab <- Phab8[complete.cases(Phab8$sockeye_km),]
dim(Phab)
...
sock <- read_csv("C:/Habitat_Diversity/Analysis/Data/stream_indicators/sock_stream_indicators.csv")
Phab <- left_join(Phab, sock, by="id_numeric")
dim(Phab)
View(Phab)
...
write.csv(pcout, file="C:/Habitat_Diversity/Analysis/Data/O2_sockeye_PCout.csv")
...
write.csv(pcvar$coord, file="C:/Habitat_Diversity/Analysis/Data/chum_indicator_scores.csv")
...
myvar<- c("slope_avgW", "slope_cvW", "elev_avgW", "elev_cvW", "forest", "shrub", "wetland", "ccomplexity", "sinuosity", "floodplainA", "flood_elev", "lake_elev", "glacierA","elev_avgS", "elev_cvS", "slope_avgS", "slope_cvS", "chum_hab", "region")

chumsub <- pcout[myvar]
chart.Correlation(chumsub, histogram=TRUE, pch=19)

fitchum <- lm(chum_hab ~ slope_avgW + slope_cvW + elev_avgW + elev_cvW + forest + shrub + wetland + ccomplexity + sinuosity + floodplainA + flood_elev + lake_elev + glacierA + elev_avgS + elev_cvS + slope_avgS + slope_cvS , data = chumsub)

stepchum <- stepAIC(fitchum, direction="both")
stepchum$anova
summary (stepchum)
...
library(car)
vif(lm(chum_hab ~ slope_avgW + elev_avgW  + forest + shrub + ccomplexity + floodplainA + lake_elev + elev_avgS
       + elev_cvS + slope_avgS, data=chumsub))
#remove elev_avgW bc VIF >5
vif(lm(chum_hab ~ slope_avgW  + forest + shrub + ccomplexity + floodplainA + lake_elev + elev_avgS
       + elev_cvS + slope_avgS, data=chumsub))

library(lme4)
fitchum2 <- lmer(chum_hab ~ slope_avgW  + forest + shrub + ccomplexity + floodplainA + lake_elev + elev_avgS
       + elev_cvS + slope_avgS + (1|region) , data = chumsub)
summary(fitchum2)
...
r2chum <- rsquared(lme4::lmer(chum_hab ~ slope_avgW  + forest + shrub + ccomplexity + floodplainA + lake_elev + elev_avgS + elev_cvS + slope_avgS + (1|region), data=chumsub))
