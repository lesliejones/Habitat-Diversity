...
#fitpink2 <- lm(pink_hab ~ PC1 + PC2 + PC3 + PC4, data = pcout)
#summary(fitpink2)

print(res.pca)

myvar<- c("slope_avgW", "slope_cvW", "elev_avgW", "wetland", "slope_avgS", "ccomplexity", "sinuosity", "flood_elev", "elev_avgS", "elev_cvS", "slope_avgS", "shrub",
             "floodplainA", "glacierA", "slope_cvS", "pink_hab", "region")

pinksub <- pcout[myvar]
chart.Correlation(pinksub, histogram=TRUE, pch=19)

fitpink <- lm(pink_hab ~ slope_avgW + slope_cvW + elev_avgW + wetland + slope_avgS + ccomplexity + sinuosity + flood_elev + elev_avgS + elev_cvS + slope_avgS + shrub +
             floodplainA + glacierA + slope_cvS , data = pinksub)
...
library(car)
vif(lm(pink_hab ~ slope_avgW + elev_avgW + wetland + ccomplexity + elev_avgS + slope_cvS, data=pinksub))

library(lme4)
fit2 <- lmer(pink_hab ~ slope_avgW + elev_avgW + wetland + ccomplexity + elev_avgS + slope_cvS + 1|region , data = pinksub)
summary(fit2)

library(piecewiseSEM)

fit3 <- lm(pink_hab ~ slope_avgW + elev_avgW + wetland + ccomplexity + elev_avgS + slope_cvS , data = pinksub)
summary(fit3)
